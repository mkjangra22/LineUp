...
function createLoaderTask(router, lane, index, tasks, semanticParent, options, retainedEnd) {
  ...
  let donor = (!preload || route.options.preload !== false) && routeLoader && !((_a = router._tx) == null ? void 0 : _a[6]) ? (_b = router._flights) == null ? void 0 : _b.get(match.id) : void 0;
  ...
  const background = !!(routeLoader && reload && match.status === "success" && !preload && !options[4] && ((typeof routeLoader === "function" ? void 0 : routeLoader == null ? void 0 : routeLoader.staleReloadMode) ?? router.options.defaultStaleReloadMode) !== "blocking");
...
async function executeClientLane(router, location, matches, options) {
  ...
  try {
    ...
    if ((_a = matched[2]) == null ? void 0 : _a.length) matched[3] = settleTasks(matched[2], void 0, void 0, reduction.then((foreground) => isControl(foreground) ? 0 : _getRenderedMatches(foreground[1]).length, () => 0));
...
async function followRedirect(router, tx, outcome) {
  ...
  if (redirect2.options.reloadDocument) {
    await router.navigate({
  ...
  const committed = router.commitLocation({
    ...location,
    viewTransition: redirect2.options.viewTransition,
    replace: true,
    resetScroll: redirect2.options.resetScroll,
    hashScrollIntoView: redirect2.options.hashScrollIntoView,
    ignoreBlocker: true
  });
  queueMicrotask(() => {
...
async function runClientTransaction(router, tx, forceStaleReload, onReady, sync, resolvedPrefix) {
  ...
  await router.startViewTransition(async () => {
    ...
    if (background == null ? void 0 : background.length) runBackground(router, tx, result[1], background, result[3]).catch(console.error);
    router.batch(() => {
...
async function loadClientRoute(router, opts) {
  ...
  const tx = [
    ...
    matches,
    Date.now(),
    Promise.resolve().then(() => runClientTransaction(router, tx, sameHref, () => offerPending(router, tx), opts == null ? void 0 : opts.sync, resolvedPrefix)).then()
  ];
  if (rematerialize) {
    tx[6] = [handoff];
...
async function preloadClientRoute(router, opts, redirects = 0, builtLocation) {
  ...
  (router._preloads ?? (router._preloads = /* @__PURE__ */ new Map())).set(controller, matches);
  let active;
  try {
    let result;
    try {
